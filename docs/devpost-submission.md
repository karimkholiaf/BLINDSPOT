...
The AI layer in `lib/ai.ts` is one fall-through chain per task. `/api/assess` leads with Claude Sonnet 5, reached through OpenRouter with the `openai` v6 client, because that route carries the product’s judgement and Sonnet is the strongest model available to us at a sane price — and because going through OpenRouter caps spend at whatever prepaid credit we bought instead of opening a billing account. `/api/extract` leads with `google/gemini-3.5-flash-lite` instead: reading material into a fixed shape rewards speed, not judgement. Behind both sit Gemini Flash models on the free tier through `@google/genai` v2.13, so that when the credit runs out the app degrades rather than going dark. A request falls through on spent credit (402), a rejected key (401/403), a rate limit (429), a server error, or a 40-second per-attempt stall cap. The one status that doesn’t fall through is 400: a malformed request is our bug, it would fail identically everywhere, and falling through would hide it.
